...
export default function QuizPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
      <div
        className="bg-card rounded-lg max-w-3xl w-full mx-4 max-h-[85vh] overflow-y-auto border border-border"
                  ...
                  return (
                    <div key={q.id} className="bg-secondary border border-border rounded-md p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <span className="text-muted-foreground/50 text-xs flex-shrink-0 ml-2">{q.points} pt{q.points > 1 ? "s" : ""}</span>
                      </div>
                      <div className="space-y-1.5 ml-1">
                        {opts.map((opt, oIdx) => (
                          ...
                          >
                            ...
                            <span className={opt.isCorrect ? "text-emerald-600 dark:text-emerald-300" : "text-muted-foreground"}>
                              {opt.text}
                            </span>
                          </div>
                        ))}
