...
export default function QuizPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={onClose}>
      <div
        className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl max-w-3xl w-full mx-4 max-h-[85vh] overflow-y-auto border border-white/10"
      ...
      >
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12 text-white/40">Failed to load quiz data.</div>
          ) : (
                    ...
                    <div key={q.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-3">
                        <p className="text-white text-sm font-medium">
                          <span className="text-white/40 mr-1.5">Q{qIdx + 1}.</span>
                          {q.text}
                        </p>
                      ...
                      <div className="space-y-1.5 ml-1">
                        {opts.map((opt, oIdx) => (
                          ...
                          >
                            ...
                            <span className={opt.isCorrect ? "text-emerald-300" : "text-white/60"}>
                              {opt.text}
                            </span>
                          </div>
                        ))}
