/**
 * POST /api/admin/courses/seed-nextjs
 *
 * Seeds (or re-seeds) the full "Next.js 16: The Complete Developer Guide" course —
...
import { getNextjsCourseSeed, buildOptions } from "@/lib/nextjs-course-seed";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const seed = getNextjsCourseSeed();
    ...
    console.error("[POST /api/admin/courses/seed-nextjs]", err);
    return NextResponse.json(
      { error: "Failed to seed the Next.js 16 course." },
      { status: 500 },
    );
  }
}
