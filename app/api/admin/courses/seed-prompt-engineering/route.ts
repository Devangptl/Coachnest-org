/**
 * POST /api/admin/courses/seed-prompt-engineering
 *
 * Seeds (or re-seeds) the full "Prompt Engineering Mastery" course —
 * category, tags, course, modules (sections), lessons, and quizzes.
 *
...
import {
  getPromptEngineeringCourseSeed,
  buildOptions,
} from "@/lib/prompt-engineering-course-seed";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const seed = getPromptEngineeringCourseSeed();

    // 1. Resolve the instructor.
    ...
    // 2. Upsert category (keyed by unique slug).
    ...
    // 3. Upsert tags (keyed by unique slug).
    ...
    // 4. Compute curriculum totals.
    const allLessons = seed.modules.flatMap((m) => m.lessons);
    const totalLessons = allLessons.length;
    const totalDuration = allLessons.reduce((sum, l) => sum + (l.duration ?? 0), 0);

    // 5. Upsert the course (keyed by unique slug). Re-seeding preserves
    //    the course row (and its enrollments/reviews) while refreshing fields.
    ...
    // 6. Reset the curriculum. Deleting lessons cascades to quizzes,
    //    questions and lesson-progress (per schema onDelete: Cascade).
    await prisma.lesson.deleteMany({ where: { courseId: course.id } });
    await prisma.section.deleteMany({ where: { courseId: course.id } });

    // 7. Re-link course tags.
    ...
    // 8. Rebuild modules → lessons → quizzes → questions.
    let lessonOrder = 0;
    let quizCount = 0;
    let questionCount = 0;
    ...
    return NextResponse.json({
      success: true,
      message: `Seeded "${seed.course.title}" — ${seed.modules.length} modules, ${totalLessons} lessons, ${quizCount} quizzes (${questionCount} questions). Attributed to ${instructor.name}.`,
    ...
    console.error("[POST /api/admin/courses/seed-prompt-engineering]", err);
    return NextResponse.json(
      { error: "Failed to seed the Prompt Engineering course." },
      { status: 500 }
    );
  }
}
