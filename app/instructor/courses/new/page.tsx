...
export default function NewInstructorCoursePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/instructor/courses", {
        ...
        body: JSON.stringify({ title, description, thumbnail: thumbnail || undefined, published, price: isFree ? null : price ? parseFloat(price) : null, isFree, level }),
      });
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/instructor/courses" className="inline-flex items-center gap-1.5 text-muted-foreground hover:text-foreground text-sm mb-6 transition-colors">
      ...
      <GlassCard>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="input-glass" placeholder="e.g. React for Beginners" required />
          </div>
          <div>
            <label className="label">Description *</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="input-glass resize-none" rows={4} placeholder="What will students learn?" required />
          </div>
          <div>
          ...
          <div className="flex items-center justify-between bg-secondary border border-border rounded-xl px-4 py-3">
            ...
            <button type="button" onClick={() => setPublished(!published)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${published ? "bg-amber-500" : "bg-white/20"}`}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Link href="/instructor/courses" className="btn-ghost border border-border">Cancel</Link>
          </div>
        </form>
      </GlassCard>
    </div>
