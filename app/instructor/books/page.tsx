    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-dashed border-white/[0.08] py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">No books yet — create your first one.</p>
        </div>
      ) : (
            ...
            <li key={b.id} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-card/50 p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {b.fileFormat} · {b.status} · {b._count.purchases} sold · ₹{Number(b.discountPrice ?? b.price ?? 0)}
                </p>
              </div>
              <Link
