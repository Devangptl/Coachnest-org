 ...
 *
 * Inserts default HTML templates for all 24 system email slugs, matching the
 * light-themed CoachNest email design. Templates use {{variable}} placeholders.
 *
 * Built-in variables injected automatically by the mailer:
 *   {{logo}}    — logo image URL
 *   {{appUrl}}  — site base URL
 *
 * Run:  npx ts-node --project tsconfig.json prisma/seed-email-templates.ts
 *   or: npx tsx prisma/seed-email-templates.ts
 */
  ...
  <table cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    ...
    <tr><td>
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr><td style="padding:0 36px 32px;">
          <a href="${ctaHref}" style="display:inline-block;background:#d97757;border:1px solid #bc6346;color:#ffffff;font-family:sans-serif;font-size:13px;font-weight:600;text-decoration:none;padding:6px 16px;border-radius:6px;">${ctaLabel} →</a>
    ...
    <tr><td style="padding:20px 0 8px;text-align:center;">
      <p style="font-family:sans-serif;font-size:11px;color:#c4bab3;margin:0 0 5px;">
        © 2026 CoachNest — Modern Learning Platform
      </p>
...
function infoTable(rows: Array<[string, string]>): string {
  const cells = rows
    ...
    .join("");
  return `
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf8f6;border:1px solid #ede8e3;border-radius:8px;margin-bottom:24px;">
            <tbody>${cells}
            </tbody>
          </table>`;
}
  ...
  return `
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
            <tr><td style="padding:18px 20px 6px;">
              <p style="font-family:sans-serif;font-size:10px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:#b0a89f;margin:0 0 14px;">What's next</p>
            </td></tr>
            ${rows}
          </table>`;
}
...
function noticeBox(text: string, color = "#d97757"): string {
  return `<div style="background:#fdf6f3;border-left:3px solid ${color};border-radius:0 6px 6px 0;padding:12px 16px;margin-bottom:24px;">
            <p style="font-family:sans-serif;font-size:13px;color:#776b5f;margin:0;line-height:1.6;">${text}</p>
          </div>`;
}

function quoteBox(label: string, content: string): string {
  return `<div style="background:#faf8f6;border:1px solid #ede8e3;border-left:3px solid #d97757;border-radius:0 6px 6px 0;padding:14px 18px;margin-bottom:24px;">
            <p style="font-family:sans-serif;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#b0a89f;margin:0 0 6px;">${label}</p>
            <p style="font-family:sans-serif;font-size:13px;color:#1c1411;margin:0;line-height:1.6;white-space:pre-wrap;">${content}</p>
          </div>`;
}
...
const templates: Array<{
  ...
  // ── 2. Subscription Activated ──────────────────────────────────────────────
  {
    ...
    subject: "Your CoachNest {{plan}} plan is now active 🎉",
    description: "Sent when a subscription is successfully activated.",
    variables: ["name", "plan", "billing"],
    htmlBody: shell(
      "Subscription confirmed",
      "You're all set, {{name}}! 🎉",
      `${para("Your <strong style=\"color:#d97757;\">{{plan}} plan</strong> is now active. Enjoy full access to your courses and start learning right away.")}
          ${infoTable([
            ["Plan", "{{plan}}"],
      ...
      "Start Learning",
      "{{appUrl}}/courses",
      `Manage your plan anytime from <a href="{{appUrl}}/dashboard/subscription" style="color:#d97757;text-decoration:none;">your subscription page</a>.`
    ),
  },
  ...
  // ── 3. Plan Changed ────────────────────────────────────────────────────────
  {
    slug: "plan-changed",
    name: "Plan Changed",
    subject: "Your CoachNest plan has been {{action}} to {{newPlan}}",
    description: "Sent when a user upgrades or downgrades their plan.",
    variables: ["name", "oldPlan", "newPlan", "billing", "action"],
    htmlBody: shell(
      "Plan updated",
      "Your plan has been {{action}}, {{name}}",
      `${para("Your subscription has been updated successfully. Your new plan is now active.")}
          ${infoTable([
          ...
          ${noticeBox("If you did not make this change, please <a href=\"{{appUrl}}/contact\" style=\"color:#d97757;\">contact support</a> immediately.")}`,
      "View My Plan",
  ...
  // ── 4. Subscription Cancelled ──────────────────────────────────────────────
  {
    ...
    htmlBody: shell(
      ...
      `${para("Hi {{name}}, your <strong style=\"color:#1c1411;\">{{plan}} plan</strong> has been cancelled. You'll retain full access until your current billing period ends.")}
          ${infoTable([
            ["Plan", "{{plan}}"],
            ["Status", "Cancelled"],
            ["Access until", "{{endDate}}"],
          ])}
          ${noticeBox("Changed your mind? You can reactivate your plan anytime before <strong>{{endDate}}</strong> and keep all your progress and enrollments.")}`,
      "Reactivate Plan",
      "{{appUrl}}/dashboard/subscription",
      `We'd love to have you back. <a href="{{appUrl}}/contact" style="color:#d97757;text-decoration:none;">Let us know</a> if there's anything we can improve.`
    ),
  },
  ...
  // ── 5. Subscription Resumed ────────────────────────────────────────────────
  {
    ...
    subject: "Your CoachNest {{plan}} plan has been reactivated ✅",
    description: "Sent when a cancelled subscription is resumed.",
    variables: ["name", "plan"],
    htmlBody: shell(
      "Welcome back",
      "Great to have you back, {{name}}! ✅",
      `${para("Your <strong style=\"color:#d97757;\">{{plan}} plan</strong> is active again. Your subscription will now renew normally — no further action required.")}
          ${infoTable([
            ["Plan", "{{plan}}"],
            ["Status", "Active"],
  ...
  // ── 6. Payment Failed ──────────────────────────────────────────────────────
  {
    ...
    htmlBody: shell(
      "Action required",
      "We couldn't process your payment ⚠️",
      `${para("Hi {{name}}, your latest subscription payment was declined. To keep your access uninterrupted, please update your billing details.")}
          ${noticeBox(
            "⚠️ If payment is not updated soon, your subscription will be suspended and you may lose access to your enrolled courses.",
            "#ef4444"
          )}`,
      "Update Payment Method",
      "{{appUrl}}/dashboard/subscription",
      `Need help? <a href="{{appUrl}}/contact" style="color:#d97757;text-decoration:none;">Contact our support team</a>.`
    ),
  },
  ...
  // ── 19. Contact Confirmation ───────────────────────────────────────────────
  {
    ...
    htmlBody: shell(
      ...
      "Visit CoachNest",
      "{{appUrl}}",
      `For urgent matters, email us directly at <a href="mailto:support@coachnest.in" style="color:#d97757;text-decoration:none;">support@coachnest.in</a>.`
    ),
  },
