...
- **Subdomain routing** (`lib/domains.ts` + `middleware.ts`): in production each org is reachable at `<slug>.coachnest.in`, where clean paths (`/`, `/login`, `/admin`, …) are rewritten into the canonical `/org/<slug>/*` routes; paths already under `/org/` pass through untouched. `org.coachnest.in` is the platform/main site (landing, registration, platform `/login` + `/admin`); the apex and `www` redirect there. An unregistered subdomain redirects to registration on the main host. Root domain comes from `NEXT_PUBLIC_ROOT_DOMAIN` (default `coachnest.in`); on localhost there are no subdomains so everything stays path-based. Requires a wildcard `*.coachnest.in` DNS record.
